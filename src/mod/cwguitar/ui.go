...
const (
	homePage = "https://www.cwguitar.cn"
)

func init() {
	config.AddConfig(CWGuitarConfig)
}

func CwguitarName() string {
	return "Cwguitar"
}
func CwguitarView() fyne.CanvasObject {
	...
	form := widget.NewForm()
	form.Append("TOKEN", widget.NewEntryWithData(binding.BindString(&CWGuitarConfig.Token)))
	ctr.Add(form)
